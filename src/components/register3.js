...
class Register3 extends Component {
    ...
    languageCheck = () => event => {
        ...
        if (this.state.LSelected[event.target.id - 1] === false) {
            ...
            this.setState({ LanguageToPractice: [...this.state.LanguageToPractice, event.target.id] }, () => {
                console.log(this.state.LanguageToPractice)
            });
            ...
            let remove = this.state.LanguageToPractice.indexOf(event.target.id);
            this.setState({
                LanguageToPractice: this.state.LanguageToPractice.filter((_, i) => i !== remove)
            },
                () => {
                    console.log(this.state.LanguageToPractice);
                }
    ...
    nextFunction = (event) => {
        ...
        var languagesKnown = this.props.location.state.languagesKnown;
        console.log(User, languagesKnown);
        ...
        let url = 'http://localhost:3001/login/register';
        ...
        fetch(url, {
            ...
            body: JSON.stringify({
                ...
                languagesToPractice: this.state.LanguageToPractice
            })
        }).then((response) => response.json())
            .then((responseJson) => {
                ...
                this.props.history.push('/register4', { user: { username: User.username, email: User.email, password: User.password, country: User.country, id: responseJson.result.insertId }, languagesKnown: languagesKnown, languagesToPractice: this.state.LanguageToPractice });
                // } else {
        ...
        return (
            ...
            <div >
                ...
                <div style={{ marginLeft: "50%", marginBottom: "45px" }}>
                    <div>
                        <h3 className="font-weight-bold my-3" style={{ color: "purple", textAlign: "center" }}>Select the languages you're Learning</h3>
                    </div>
